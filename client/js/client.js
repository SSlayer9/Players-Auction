	...
	switch (type) {
		...
		case "ASSIGN-PLAYER":
			...
			for (var p of players) {
				html += '<option value="' + Tools.toPlayerId(p) + '">' + Tools.escapeHTML(p) + '</option>';
			}
			html += '</select></p>';
			html += '<p><span class="field-name">Money (K): </span>' + 
		...
		case "DELETE-CAPTAIN":
			...
			for (var c of etc.captains) {
				html += '<option value="' + Tools.toId(c) + '">' + Tools.escapeHTML(c) + '</option>';
			}
			html += '</select></p>';
			html += '</form>';
			break;
...
Client.prototype.completeLine = function (line, room) {
	...
	if (line.indexOf(" ") === -1 && line.charAt(0) === "/") {
		...
		if (!this.home_room || !this.home_room.room_list) return list;
		var rooms = [];
		...
		for (var p in room.auction.players) {
			if (p.substr(0, subPlayer.length) === subPlayer) {
				players.push(room.auction.players[p].name);
			}
		}
		if (players.length === 0) {
			return line;
